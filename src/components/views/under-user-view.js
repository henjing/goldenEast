	...
	render(){
		...
		return (
			<Table columns={columns} dataSource={data} bordered 
					title={tableTitle} pagination={false} size="middle" style={{marginBottom: '15px'}}/>
		)
	},
});
...
export const UnderUserTable = React.createClass({
	...
	render(){
		const data = this.props.data;
		const columns = this.getColumns();
		const tableTitle = () => (<div style={{textAlign: 'center',color: '#333',fontSize: '14px'}}>一度人脉</div>);
		const pagination = {
	        defaultPageSize : 12,
		...
		return (
			<Table columns={columns} dataSource={data} bordered title={tableTitle} pagination={pagination} size="middle"/>
		)
	},
});
